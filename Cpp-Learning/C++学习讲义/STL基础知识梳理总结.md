...
`vector<类型> 容器名(count)` ：初始化大小为count，默认用0填充。
...
`容器名.resize(count)` ：重新指定大小为count。如果重新指定的比原来长，默认用0填充新的位置，如果重新指定的比原来短，超出部分会删除掉。
...
`list<类型> 容器名(count)` ：初始化大小为count，默认用0填充。
...
`容器名.resize(count)` ：重新指定大小为count。如果重新指定的比原来长，默认用 `\0` 填充新的位置；如果重新指定的比原来短，超出部分会删除掉。
