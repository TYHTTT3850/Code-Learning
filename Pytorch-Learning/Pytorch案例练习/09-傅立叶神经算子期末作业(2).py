...
def exact_solution(x, t):
    ...
    保证输入输出一一对应，不改变顺序。
    """
    phi1 = phi_i(x, t, ci=c1, xi=x_1, A=a)
    phi2 = phi_i(x, t, ci=c2, xi=x_2, A=a)
    U = phi1 + phi2
    return U

def generate_dataset(n_samples=1000, Resolution=128):
    data_U_in = []  # 输入函数 u(x, 0)
    data_U_out = []  # 输出函数 u(x, 1)

    for _ in range(n_samples):
        x = torch.linspace(0, a, Resolution)  # x从0到a，分辨率为resolution
        U_in = exact_solution(x,torch.zeros_like(x))  # u(x, 0)
        U_out = exact_solution(x,5*torch.ones_like(x))  # u(x, 5)
        data_U_in.append(U_in)
        data_U_out.append(U_out)

    return torch.stack(data_U_in), torch.stack(data_U_out)
...
class SpectralConv1d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, modes):
        ...
        self.weight = nn.Parameter(torch.randn([in_channels, out_channels, modes],
                                               dtype=torch.cfloat) * scale)  # 取复数类型的浮点数。初始化一个三维张量 weight，表示在频域里，用于每个频率分量的线性变换参数。[in_channels, out_channels, modes]
    ...
    def forward(self, x):
        ...
        for i in range(self.modes):
            ...
            self.weight[:, :, i]表示所有样本，所有通道的第i个频率分量权重。
...
idx = 0  # 可以改成其他样本编号(其实都一样，因为每个样本都是同一个函数组，哈哈哈🤪)
u0 = u0.cpu()
u1 = u1.cpu()
prediction_test = prediction_test.cpu()
plt.figure(figsize=(8, 5))
plt.plot(np.linspace(0, 30, resolution), u1[idx].numpy(), '--', color='green', label="exact u(x)")
plt.plot(np.linspace(0, 30, resolution), prediction_test[idx].numpy(), 'r-', alpha=0.5, label="prediction u(x)")
plt.legend()
plt.savefig("FNO Solution2.pdf", format="pdf")
plt.show()